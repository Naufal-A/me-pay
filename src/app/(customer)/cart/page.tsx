...
export default function CartPage() {
  ...
  const { items, removeItem, clearCart, totalPrice, tableNumber } = useCartStore();
  ...
  return (
    <main className="bg-gray-50 min-h-screen font-sans">
      ...
      <header className="bg-white px-4 py-4 flex items-center gap-3 border-b border-gray-100">
        ...
        <div>
          <h1 className="text-lg font-bold text-gray-900 leading-tight">Kantinita</h1>
      ...
      <div className="px-4 pt-6 pb-40 flex flex-col gap-6 max-w-lg mx-auto">
        ...
        {/* Ringkasan Pesanan */}
        <section>
          <h2 className="text-xl font-bold text-gray-900 mb-3">Ringkasan Pesanan</h2>
        ...
        {/* Metode Pembayaran */}
        <section>
          <h2 className="text-xl font-bold text-gray-900 mb-3">Metode Pembayaran</h2>
          <div className="flex flex-col gap-3">
            ...
            >
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                ...
                <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 ${paymentMethod === "qris" ? "text-red-600" : "text-gray-500"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              ...
              <div className="flex-1">
                <p className={`font-semibold text-sm ${paymentMethod === "qris" ? "text-red-700" : "text-gray-800"}`}>
                  QRIS / E-Wallet
                </p>
                <p className="text-xs text-gray-500 mt-0.5">Bayar otomatis via aplikasi</p>
              </div>
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${
                paymentMethod === "qris" ? "border-red-500" : "border-gray-300"
              }`}>
                {paymentMethod === "qris" && <div className="w-2.5 h-2.5 rounded-full bg-red-500" />}
              </div>
            </button>
            ...
            >
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                ...
                <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 ${paymentMethod === "kasir" ? "text-red-600" : "text-gray-500"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z" />
                </svg>
              </div>
              <div className="flex-1">
                <p className={`font-semibold text-sm ${paymentMethod === "kasir" ? "text-red-700" : "text-gray-800"}`}>
                  Bayar di Kasir
                </p>
                <p className="text-xs text-gray-500 mt-0.5">Tunai atau kartu debit</p>
              </div>
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${
                paymentMethod === "kasir" ? "border-red-500" : "border-gray-300"
              }`}>
                {paymentMethod === "kasir" && <div className="w-2.5 h-2.5 rounded-full bg-red-500" />}
              </div>
            </button>
          </div>
        </section>
